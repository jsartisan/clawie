#!/bin/bash
# Build the Clawie agent container image.
#
...
# Derive the image name from the project root so two Clawie installs on the
# same host don't overwrite each other's `clawie-agent:latest` tag. Matches
...
echo "Building Clawie agent container image..."
