...
# setup.sh — Bootstrap script for Clawie
...
# Where verbose bootstrap logs go. clawie.sh captures setup.sh's stdout to
...
# Default: write to the raw bootstrap log if clawie.sh pointed us there,
# else fall back to a dedicated bootstrap log (keeps standalone `bash
# setup.sh` invocations working).
LOG_FILE="${CLAWIE_BOOTSTRAP_LOG:-${PROJECT_ROOT}/logs/bootstrap.log}"
